...
|`at`|_string_|_path_|_optional_|Used to get the balance at a specific point in history (ignoring any later transactions). Formatted as an ISO 8601 string.
|===
...
|`includeTransactionType`|_array_|_path_|_optional_|Which transaction types to include. Multiple values allowed. See table _"Valid transaction types"_ below.
...
|===
NOTE: *Deprecated* transaction types are no longer in use, but may appear in historic data._
...
|`organizationUUID`|_string_|_path_|_required_|Organization identifier as UUID or "self" to denote that the organization should be derived from the authenticated user.
|`at`|_string_|_path_|_optional_|Used to get payouts until a specific historic date. Formatted as an ISO 8601 string.
|===
