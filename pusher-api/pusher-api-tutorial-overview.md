...
When an event happens, the pusher API sends a `POST` request with the activity information in a payload to the HTTPS endpoint.
...
```
For example, if you subscribe to the `InventoryTrackingStarted` event, when that event happens at the Zettle Go app, you will receive a notification as follows:
...
> **Note:** As payloads may be updated due to changes of the other APIs, you can ignore unknown fields.
